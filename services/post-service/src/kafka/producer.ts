...
export async function initProducer(): Promise<void> {
  ...
  while (attempt < maxRetries) {
    try {
      ...
      console.log('[Post Service] Kafka producer connected');
      return;
    } catch (error) {
      attempt++;
      console.warn(`[Post Service] Kafka connection attempt ${attempt}/${maxRetries} failed. Retrying in 3s...`);
...
export async function publishEvent(topic: string, payload: object): Promise<void> {
  ...
  await producer.send({
    ...
    messages: [
      {
        key: String((payload as Record<string, unknown>).postId || Date.now()),
